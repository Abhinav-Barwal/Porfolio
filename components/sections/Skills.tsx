import Image from "next/image";

export default function Skills() {
    return(
        <section id="skills">
            <div className="container">
                    <div className="seperator" data-aos="fade-up" data-aos-duration="2000">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="170" height="12" viewBox="0 0 170 12" fill="none">
                        <rect width="170" height="12" fill="url(#pattern0_2_335)"/>
                        <defs>
                        <pattern id="pattern0_2_335" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlinkHref="#image0_2_335" transform="scale(0.00588235 0.0833333)"/>
                        </pattern>
                        <image id="image0_2_335" width="170" height="12" preserveAspectRatio="none" xlinkHref="data:image/png;base64,"/>
                        </defs>
                        </svg>
                    </div>
                <div className="title">
                    <h2 data-aos="zoom-in">SKILLS</h2>
                </div>
                <h3>USING NOW:</h3>
                <div className="skills-cont">
                    <div className="box flex flex-col items-center">
                        <Image src="/images/html.png" alt="html5" width={80} height={100}></Image>
                        <span>HTML5</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/css.png" alt="html5" width={80} height={100}></Image>
                        <span>CSS3</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/sass.png" alt="html5" width={120} height={140}></Image>
                        <span>SASS</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/js.png" alt="html5" width={90} height={110}></Image>
                        <span>JAVASCRIPT</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/react.png" alt="html5" width={85} height={100}></Image>
                        <span>REACT</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/bootstrap.png" alt="html5" width={80} height={100}></Image>
                        <span>BOOTSTRAP</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/git.png" alt="html5" width={80} height={100}></Image>
                        <span>GIT</span>
                    </div>
                    <div className="box flex flex-col items-center">
                        <Image src="/images/figma.png" alt="html5" width={60} height={80}></Image>
                        <span>FIGMA</span>
                    </div>
                </div>
            </div>
        </section>
    )
}